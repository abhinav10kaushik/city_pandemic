...
"""
    There are six different simulations that are 'built in' you can create more if you wish :D
"""
...
class City(object):
    ...
    def run_turn(self, turn_number):
        ...
        self.move_infected(num_per_neighbour, turn_number)
    ...
    def move_infected(self, num_per_neighbour, turn_number):

        for nbr in self.neighbours:
            nbr.incoming_infected += num_per_neighbour
            self.infected -= num_per_neighbour

        cities = ['Sydney', 'Newcastle', 'Canberra']
        if (self.infected > 0) and (self.name not in city_name):
            city_name.add(self.name)
            LOG_FILE.write("Iteration # {} \n".format(str(turn_number)))
            # || City - {} || Infected - {}     , self.name, self.infected
        if self.infected > 5:
            num_resolved = max(self.infected // AVERAGE_DURATION, 5)
...
if __name__ == "__main__":
    ...
    cities = get_city_data("city_data.csv")
    treatments = dict()
